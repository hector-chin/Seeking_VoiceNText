import json
import pandas as pd
with open('./YoutubeWordSegment.json') as f:
    youdata = json.loads(f.read())
with open('./PodcastWordSegment.json') as f:
    poddata = json.loads(f.read())


with open('./NewYoutubeWordSegment_v1.json') as f:
    newyou = json.loads(f.read())

with open('./NewPodcastWordSegment_v1.json') as f:
...
print(len(youdata))
print(len(poddata))
with open('./YoutubeWordSegment_v1.json', 'w') as f:
    json.dump(youdata, f)
with open('./PodcastWordSegment_v1.json', 'w') as f:
    json.dump(poddata, f)