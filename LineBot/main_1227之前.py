...
from sqlsetting import query_mysql, insert1_mysql, insert2_mysql, update1_mysql, update2_mysql, query_search_record, update0_mysql, update_YT_mysql, update_PC_mysql, update_BK_mysql, update0_mysql_2
...
def handle_message(event):
    ...
    if mtext == '@精準搜尋' or mtext == '@其他精準搜尋':    # 當收到'@精準搜尋', 連到資料庫查詢當前設定, 並修改設定為1
        # try:
        userid, query_data = query_mysql(event)  # 查詢mysql資料庫
        if query_data == None:
            insert1_mysql(event)
        elif query_data[0] == '1':
            pass
            ...
            update1_mysql(event)
        message = TextSendMessage(text="請輸入要精準搜尋的詞彙:")
        ...
        try:
            userid, query_data = query_mysql(event)  # 查詢資料庫
...
#######################################################################################################################
        ...
        # try:
        userid, query_data = query_mysql(event)  # 查詢資料庫的FUNC設定
...
########################################################################################################## 第一次精準搜尋▼
        elif query_data[0] == '1':        # 將 mtext 做精準搜尋
            update0_mysql(event)
            youtube_query_data = mongo_youtube_query(event, mtext)
            podcast_query_data = mongo_podcast_query(event, mtext)
            book_query_data = mongo_book_query(event, mtext)
...
############################################################################################ 第一次關聯搜尋▼
        elif query_data[0] == '2':   # 將 mtext 拿去做關聯搜尋
            ...
            if len(youtube_result) < 3:
                ...
                new_result = [(ti[str(document_number + 3378)]['Title'], score, ti[str(document_number + 3378)]['Url'])
                              for document_number, score in sorted(enumerate(new_sim[3378:5540]),
                                                                   key=lambda x: (x[1], int(ti[str(x[0] + 3378)]['Date'])),
                                                                   reverse=True)[:top] if score > 0]
                youtube_result.extend(new_result)

            youtube_result = youtube_result[0:3]

            ############################################################################################ 第一次關聯搜尋▼podcast

            podcast_result = [(ti[str(document_number + 5540)]['Title'], score, ti[str(document_number + 5540)]['Url'])
                              for document_number, score in sorted(enumerate(sim[5540:7513]),
            ...
            if len(podcast_result) < 3:
                ...
                new_result = [(ti[str(document_number + 5540)]['Title'], score, ti[str(document_number + 5540)]['Url'])
                              for document_number, score in sorted(enumerate(new_sim[5540:]),
                                                                   key=lambda x: (x[1], int(ti[str(x[0] + 5540)]['Date'])),
                                                                   reverse=True)[:top] if score > 0]
                podcast_result.extend(new_result)
    ...
    #################################################################################################### 看更多▼
        ...
        try:
            FUNC_SETTING, SEARCH_TEXT_RECORD, YT_RECORD, PC_RECORD, BK_RECORD = query_search_record(event)  # 查詢資料庫

            if FUNC_SETTING == '1':    # 將 mtext 做精準搜尋
                NEW_YT_RECORD = YT_RECORD + 1            # 這次的次數等於上次的次數+1
                update_YT_mysql(event, NEW_YT_RECORD)    # 將這次的次數寫回mysql

                youtube_query_data = mongo_youtube_query_re(event, SEARCH_TEXT_RECORD,NEW_YT_RECORD)
        ...
        try:
            ...
            if FUNC_SETTING == '1':    # 將 mtext 做精準搜尋
                ...
                podcast_query_data = mongo_podcast_query_re(event, SEARCH_TEXT_RECORD,NEW_PC_RECORD)
                ...
                podcast_result = [
                    (ti[str(document_number + 5540)]['Title'], score, ti[str(document_number + 5540)]['Url'])
                    for document_number, score in sorted(enumerate(sim[5540:7513]),
                                                         key=lambda x: (x[1], int(ti[str(x[0] + 5540)]['Date'])),
                                                         reverse=True)[:top] if score > 0.3]
        ...
        try:
            ...
            if FUNC_SETTING == '1':    # 將 mtext 做精準搜尋
                ...
                book_query_data = mongo_book_query_re(event, SEARCH_TEXT_RECORD,NEW_BK_RECORD)
                if len(book_query_data) == 0:
                    book_text = "查無相關結果\n"
                elif len(book_query_data) == 1:
                    book_text = "{}\n{}".format(
    ...
    ############################################################################################################# 看更多▲
        ...
        try:
            ...
            message = [  # 串列
                ...
                ImageSendMessage(
                    original_content_url= baseurl + 'cloudphoto/' + photo_num + '.jpg',
                    preview_image_url= baseurl + 'cloudphoto/' + photo_num + '.jpg'
                )
            ]
